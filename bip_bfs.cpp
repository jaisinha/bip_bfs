...
using namespace std;
bool bipar(int source, int colour[], vector<int> v[])
{
    queue<int> q;
    q.push(source);
    colour[source] = 1;
...
bool checkbipar(vector<int> v[], int n)
{
    int colour[n];
    mem1(colour);  // memory set(not imp to make here par bna lo it will make the code look good and simple)
    for (int i = 0; i < n; i++)
    {
        if (colour[i] == -1)
        {
